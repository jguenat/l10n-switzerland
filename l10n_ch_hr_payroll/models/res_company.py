...
class LPPContract(models.Model):
    ...
    company_id = fields.Many2one(
        string="Company", invisible=True, comodel_name="res.company"
    )
    ...
    dc_amount = fields.Float(string="Deduction Amount", required=True)
...
class ResCompany(models.Model):
    ...
    fa_min_number_childs = fields.Integer(
        string="Additional allowance for the", default="3", required=False
    )
    ...
    yearlySalaries = fields.Integer(
        string="Number of salaries per year", default=12, required=False
    )
