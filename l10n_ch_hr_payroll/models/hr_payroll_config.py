...
class HrPayrollConfig(models.TransientModel):
    ...
    def assign_account_to_rule(self, rules, account, type):
        if account.id:
            ...
            rules = self.env["hr.salary.rule"].search([("id", "in", ids)])
            for rule in rules:
                if type == "debit":
    ...
    def save_configs(self):
        for config in self:
            ...
            config.assign_account_to_rule(
                ["l10n_ch_hr_payroll.NET_CH"], config.cc, "debit"
            )
            ...
            # net
            config.assign_account_to_rule(
                ["l10n_ch_hr_payroll.NET_CH"], config.net, "credit"
            )
            # provision 13
            config.assign_account_to_rule(
                ["l10n_ch_hr_payroll.PROVISION_13"],
            ...
            config.assign_account_to_rule(
                ["l10n_ch_hr_payroll.FADMIN"], config.avs_d, "credit"
            )
            ...
            # lpp_d
            config.assign_account_to_rule(
                ["l10n_ch_hr_payroll.LPP_C", "l10n_ch_hr_payroll.LPP_E"],
                config.lpp_d,
                "debit",
            )
            ...
            # lpp_c
            config.assign_account_to_rule(
                ["l10n_ch_hr_payroll.LPP_C"], config.lpp_c, "credit"
            )
            ...
            # staff_ins
            config.assign_account_to_rule(
                ["l10n_ch_hr_payroll.LAA_C", "l10n_ch_hr_payroll.LCA_C"],
                config.staff_ins,
                "credit",
            )
            ...
            # other_costs
            config.assign_account_to_rule(
                ["l10n_ch_hr_payroll.FADMIN"], config.other_costs, "debit"
            )
